...
const sampleServerMatch: MswMatch[] = [{
  "request": {
...
const sampleWorkerMatch: MswMatch[] = [{
  "request": {
    "id": "de5eefb0-c451-4ae2-9695-e02626f00ca7",
    "url": new URL("http://localhost:8081/products"), "method": "GET",
    "body": undefined,
  ...
  "body":JSON.stringify([{ id: "09", type: "CREDIT_CARD", name: "Gem Visa" }]),
  "headers":
  new Headers({ "accept": "application/json, text/plain, */*", "authorization": "Bearer 2022-03-01T19:36:18.277Z", "user-agent": "axios/0.21.1", "host": "localhost:8081", "content-type": "application/json", "x-powered-by": "msw" })
}]
describe("writes an msw req/res to a pact", () => {
  it("should convert an msw server match to a pact", async () => {
    expect(
      convertMswMatchToPact({
        matches: sampleServerMatch as any,
  ...
  it("should convert an msw worker match to a pact", async () => {
    expect(
      convertMswMatchToPact({
        matches: sampleWorkerMatch as any,
        consumer: 'interaction.consumer.name',
        provider: 'interaction.provider.name',
        isWorker: true
      })
