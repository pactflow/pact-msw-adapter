...
describe("API - With MSW mock generating a pact", () => {
	...
	test("post product ID 10", async () => {
		...
		server.use(
			http.post(`${API.url}/product/10`, () => HttpResponse.json(productData)),
		);
	...
	test("get product ID 10", async () => {
		...
		server.use(
			http.get(`${API.url}/product/10`, () => HttpResponse.json(product)),
		);
	...
	test("handle requests mocked with error", async () => {
		server.use(http.get(`${API.url}/product/10`, () => HttpResponse.error()));
		...
		await expect(() => API.getProduct("10")).rejects.toEqual(
			new Error("TypeError: Network error"),
		);
	});

	test("unhandled route", async () => {
		await expect(API.getProduct("11")).rejects.toThrow(ECONNREFUSED_RE);
	});
